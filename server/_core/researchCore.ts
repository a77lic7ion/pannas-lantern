// Shared web-research core — used by BOTH deployments:
//  - LAN:    server/_core/researchProxy.ts (Express handler) imports from here
//  - Vercel: api/research.ts (serverless function) imports from here
// Same-origin proxy for web research. The browser cannot call DuckDuckGo
// directly (CORS), so the client hits /api/research?q=... and the server
